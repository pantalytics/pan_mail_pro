...
19.0.6.4.0 removes both ways of switching the internal filter off: the global
`sync_internal_email` parameter and the per-mailbox `exclude_internal` boolean.
...
def migrate(cr, version):
    ...
    env['ir.config_parameter'].sudo().search(
        [('key', '=', PARAM_SYNC_INTERNAL)]).unlink()
