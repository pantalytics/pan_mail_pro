...
export class ConversationView extends Component {
    ...
    async setFolder(folder, mailboxId) {
        ...
        // bar feel like it undoes itself.
     ...
     * the list takes.
     *
     * The domain is the whole of it, bar one thing a domain cannot say: mail
     * filed on nothing is not one conversation, so the filter that asks for
     * it carries `pan_mail_ungrouped` in its own context and the list stops
     * grouping. That is a search view attribute, not a special case in here.
     */
    searchArgs() {
        return {
