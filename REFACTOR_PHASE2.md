...
leaving it for someone to remember. The script shipped as `migrations/19.0.2.1.0/post-migrate.py`:
when this branch merged back into `19.0` the mainline had already moved to 19.0.2.0.1, and Odoo only
runs a migration whose version is *higher* than the installed one — at the originally planned
`19.0.1.2.0` it would never have run at all.
